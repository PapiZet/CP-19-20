...
tar = cataExp g where
  g =  either vazio nVazio


vazio x = [("",x)]


nVazio (o,l) = map(\ (a,b) -> (o++a,b)) (juntaL l)
...
juntaL :: [[a]] -> [a]
juntaL [] = []
juntaL (h:t) = h ++ juntaL t

\end{code}

Analisando o módulo List.hs, encontrámos a função look, que usámos na definição seguinte:

\begin{code}

dic_rd p = look p . dic_exp

\end{code}

\begin{code}

dic_in = myCurry (hyloExp dictIn searchIn)

\end{code}

Esta função faz a procura de uma palavra num dicionário e daí,
poe a sua tradução (se já existir uma palavra igual) ou
acrescenta à estrutura os restantes caracteres da palavra e a respetiva tradução

\begin{code}


searchIn :: (String,String, Dict) -> Either Dict (String, [(String, String, Dict)])

searchIn (pal, trad, (Term o l))  | pal == "" = put
                                  | x == o = if y == "" then put else Right(x,map(\a -> (y, trad, a)) l)
                                  | otherwise = Left (Term o l)


                              where (x,y) = splitAt (length o) pal
                                    put = Left (Term o (Var trad:l))
searchIn (_,trad,v) = Left v


\end{code}

Esta função é uma especificação do In de expressões para o caso em que recebemos Either Dict (String,[Dict])

\begin{code}

dictIn :: Either Dict (String,[Dict]) -> Dict
dictIn = either id (uncurry Term)


\end{code}

Esta função recebe uma função e 3 variáveis e aplica a função a essas variáveis.

\begin{code}

myCurry :: ((a,b,c) -> d) -> a -> b -> c -> d
myCurry f a b c = f(a,b,c)
...
\begin{eqnarray*}
\xymatrix@@C=3cm{
    |Dict + (String x (String x String x Dict)*)|
    \ar[d]_-{id + id * map (anag)}
&
    |String x String x Dict|
    \ar[d]_-{(anag)}
    \ar[l]_-{g}
\\
    |Dict + (String x Dict*)| 
    \ar[d]_-{id + id * map (catah)}
    \ar@@/^1pc/[r]^{|out|}
    \ar@@{}[r]||-{\cong}
&
    |Dict|
...
maisDir = cataBTree g
  where g = either (const Nothing) f where
        f (x,(e,Nothing)) = Just x
        f (_,(_,d)) = d

maisEsq = cataBTree g
  where g = either (const Nothing) f where
        f (x,(Nothing,d)) = Just x
        f (_,(e,_)) = e
...
insOrd x (Node (n, (e, d))) | x == n = (Node (x, (e, d)))
                            | x < n  = (Node (n, ((insOrd x e), d)))
                            | x > n  = (Node (n, (e, (insOrd x d))))

isOrd' = cataBTree g
  where g = undefined

isOrd Empty = True
isOrd (Node (n, (Empty, Empty))) = True
isOrd (Node (n, (Node (l, (b,c)), Empty))) = if (n > l) then isOrd (Node (l, (b,c))) else False
isOrd (Node (n, (Empty, Node (r,(d,e))))) = if (n < r) then isOrd (Node (r, (d,e))) else False
isOrd (Node (n, (Node (l, (b,c)), Node (r,(d,e))))) = if (n > l) && (n < r) then isOrd (Node (l, (b,c))) && isOrd (Node (r, (d,e))) else False

rrot Empty = Empty
rrot (Node (n, (Empty,d))) = (Node (n, (Empty,d)))
rrot (Node (n, (Node (e, (ee, ed)), d))) = (Node (e, (ee, Node (n, (ed,d)))))

lrot Empty = Empty
lrot (Node (n, (e,Empty))) = (Node (n, (e,Empty)))
lrot (Node (n, (e,(Node (d, (de,dd)))))) = (Node (d , (Node (n, (e ,de)), dd)))

splay [] a = a
splay _ Empty = Empty
splay (h:t) (Node (n, (e, d))) = if h then splay t e else splay t d
...
Analisando o módulo BTree.hs e as suas funções definidas, facilmente percebemos que as funções in e out de árvores binárias e de árvores de decisões binárias não são muito diferentes. Assim, após interpretarmos durante algum tempo estas definições, chegamos à seguinte conclusão:
...
outBdt (Dec a) = Left (a)
outBdt (Query (l,(t1,t2))) = Right(l,(t1,t2))
...
Para fazer o diagrama da anaBdt, começamos primeiro por estudar o catamorfismo de uma árvore binária de decisão:
...
Um anamorfismo também é caraterizado por uma propriedade universal, mas aplicamos uma inversão do sentido das setas:
...
navLTree = cataLTree g 
  where g = either (flip (const Leaf)) (curry f)
          where f ((e,d) , []) = Fork(e [] , d [])
                f ((e,d) , (h:t)) = if h then e t else d t
...
bnavLTree = cataLTree g
  where g = either (flip (const Leaf)) (curry f)
          where f ((e,d),Empty) = Fork(e Empty, d Empty)
                f ((e,d),(Node (n, (e2, Empty)))) = if n then e e2 else d Empty
                f ((e,d),(Node (n, (Empty, d2)))) = if n then e Empty else d d2
